...
while True:
    response = post('https://api.igdb.com/v4/games', **{'headers': {'Client-ID': os.getenv('CLIENT_ID'),
                                                                    'Authorization': os.getenv('AUTORIZATION')}, 'data': 'fields franchises; limit 500; offset ' + str(offset) + '; where franchises != null;'})
    ...
    offset += 500
    franchises.extend(parseData(res))
...
insert("INSERT INTO BD2.dbo.franchise_member(franchise_member_id, franchise_id, game_id)VALUES(%d, %d, %d)", franchises)

end = time.time()
print('Franchise Members Added: ' + str(len(franchises)))
