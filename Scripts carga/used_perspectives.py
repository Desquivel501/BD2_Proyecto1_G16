...
def parseData(game_list):
    ...
    for game in game_list:
        ...
        for perspective in game['player_perspectives']:
            new_franchises.append((counter, perspective, game['id']))
            counter += 1
...
while True:
    response = post('https://api.igdb.com/v4/games', **{'headers': {'Client-ID': os.getenv('CLIENT_ID'),
                                                                    'Authorization': os.getenv('AUTORIZATION')}, 'data': 'fields player_perspectives; limit 500; offset ' + str(offset) + '; where player_perspectives != null;'})
    ...
    offset += 500
    perspectives.extend(parseData(res))
...
insert("INSERT INTO BD2.dbo.used_perspective(used_p_id, perspective_id, game_id )VALUES(%d, %d, %d)", perspectives)

end = time.time()
print('Used Perspectives Added: ' + str(len(perspectives)))
